...
| `sources/dart.py` | 663 | DART 배당 공시 — 목록·본문·파싱·배당락일 역산 (§13) |
| `backfill.py` | 304 | 러너·재개·휴장 확정·CLI |
| `preprocess.py` | 320 | 수정주가·이벤트 분류·상폐 목록 |
| `dividend.py` | 335 | 배당 러너·재개·재파싱·CLI (§13) |
...
배당 결의는 달이 정해져 있다 — 결산배당 1~3월, 분기·중간배당 4·7·10월, 이사회를 미리
여는 곳이 12월. 그 달만 훑으면 하루 안에 끝난다. 어느 달을 훑었는지 `ingest_day` 에
남으므로 나중에 달을 늘리면 **안 훑은 달만** 추가로 받는다.
...
| 🔴 | 현물배당·주식배당은 서식이 달라 **금액 칸을 못 읽는다** — `needs_review` 로 남는다 |
