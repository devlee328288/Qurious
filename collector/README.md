...
```mermaid
flowchart LR
  PD["price_daily<br/>원본 종가 clpr"] --> PA["price_adjusted<br/>adj_clpr · PR"]
  PD -. "같은 날 종가로<br/>재투자 가격" .-> TR
  DV["dividend<br/>ex_div_dt · dps"] --> TR["price_total_return<br/>tr_index · tr_index_net · pr_index"]
...
```
배당 기여 (TR ÷ PR − 1) · 6년 9개월 누적

INVENI      015360  │████████████████████████████████████████│ +80.55%p
삼성카드     029780  │██████████████████████████████████████  │ +77.11%p
서호전기     065710  │███████████████████████████████████     │ +71.64%p
크레버스     096240  │█████████████████████████████████       │ +66.26%p
동남합성     023450  │█████████████████████████████           │ +59.36%p
...
현대차       005380  │█████████████                           │ +25.98%p
삼성전자     005930  │████████                                │ +16.60%p
SK하이닉스   000660  │███                                     │  +6.52%p
카카오       035720  │▌                                       │  +0.61%p
                    └────────┴────────┴────────┴────────┴────┘
                    0       20       40       60       80  (%p)
```

> 🔴 배당 기여가 **0.61%p ~ 80.55%p** 로 벌어진다. **PR 끼리 비교하면 고배당주가
> 실제보다 나쁘게 나오고, 그 사실 자체를 PR 로는 알 수 없다.**

### 적용/미적용 내역 — 하나도 조용히 버리지 않는다

| 사유 | 건수 | 성격 |
|---|---:|---|
| 배당락일이 2019년 이전 | 1,036 | 🟢 시세가 2020-01-02 부터라 우리 구간 밖이다 |
| `dps` 가 NULL | 49 | 🟡 현물배당 서식 등. 17건은 `needs_review` |
| `dps` 가 0원 | 3 | 🟡 |
| 정정공시 중복 | 2 | 🟢 접수번호가 큰 쪽만 남겼다 |
| 그날 시세 없음 (상장 전 등) | 14 | 🟢 대부분 상장 직후 기업 |
| **두 신호가 어긋나 적용 안 함** | **1** | 🔴 자회사 배당 (위) |
